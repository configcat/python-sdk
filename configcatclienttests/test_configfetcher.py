...
class ConfigFetcherTests(unittest.TestCase):
    ...
    def test_server_side_etag(self):
        ...
        fetch_response = fetcher.get_configuration_json()
        self.assertNotEqual(fetcher._etag, '')
        logging.warning('ETag: {}'.format(fetcher._etag))
        self.assertTrue(fetch_response.is_fetched())
        self.assertFalse(fetch_response.is_not_modified())
        fetch_response = fetcher.get_configuration_json()
        logging.warning('Request header If-None-Match: {}'.format(fetch_response._response.request.headers._store.get('if-none-match')[1]))
        logging.warning('Response status_code: {}'.format(fetch_response._response.status_code))
        logging.warning('Response headers: {}'.format(fetch_response._response.headers))

        self.assertNotEqual(fetch_response._response.request.headers._store.get('if-none-match')[1], '')
